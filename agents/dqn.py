    ...
    def make_decision(self, obs):
        ...
        # a = np.argmax(self.q(obs))
            ...
            act = tf.random.uniform(
                shape=(), minval=0, maxval=self.num_act, dtype=tf.int64
            )
    ...
    def train_one_batch(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            pred_qval = tf.math.reduce_sum(
                self.qnet(data["obs"]) * tf.one_hot(data["act"], self.num_act), axis=-1
            )
            targ_qval = data["rew"] + self.gamma * (
                1 - data["done"]
            ) * tf.math.reduce_sum(
                self.targ_qnet(data["nobs"])
                * tf.one_hot(
                    tf.math.argmax(self.qnet(data["nobs"]), axis=1), self.num_act
                ),
                axis=1,
            )  # double DQN trick
